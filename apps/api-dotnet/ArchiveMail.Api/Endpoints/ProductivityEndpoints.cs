...
public static class ProductivityEndpoints
{
    public static IEndpointRouteBuilder MapProductivityEndpoints(this IEndpointRouteBuilder app)
    {
        ...
        { try { return Results.Json(await repository.CreateDraftAsync(request, Session(context).User.Id, token), statusCode: 201); } catch (Exception error) { return Error(error); } })
        ...
        { try { return Results.Ok(await repository.UpdateDraftAsync(draftId, request, Session(context).User.Id, token)); } catch (Exception error) { return Error(error); } })
        ...
        { try { await repository.DeleteDraftAsync(draftId, Session(context).User.Id, token); return Results.NoContent(); } catch (Exception error) { return Error(error); } })
        ...
        { if (!Admin(context)) return Results.Forbid(); try { return Results.Json(await repository.CreateReplyStyleAsync(request, token), statusCode: 201); } catch (Exception error) { return Error(error); } })
        ...
        { if (!Admin(context)) return Results.Forbid(); try { return Results.Ok(await repository.UpdateReplyStyleAsync(styleId, request, token)); } catch (Exception error) { return Error(error); } })
        ...
        { if (!Admin(context)) return Results.Forbid(); try { await repository.DeleteReplyStyleAsync(styleId, token); return Results.NoContent(); } catch (Exception error) { return Error(error); } })
        ...
        { if (!Admin(context)) return Results.Forbid(); try { await repository.DeleteResumeAsync(resumeId, token); return Results.NoContent(); } catch (Exception error) { return Error(error); } })
