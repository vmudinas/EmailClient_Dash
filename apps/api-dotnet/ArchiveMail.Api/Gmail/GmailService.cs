    ...
    ILogger<GmailService> logger)
{
    ...
    {
        var configured=settings.Current().GmailValue;if(string.IsNullOrWhiteSpace(configured.ClientId)||string.IsNullOrWhiteSpace(configured.ClientSecret))throw new InvalidOperationException("Configure Gmail OAuth credentials in Admin settings first");
    ...
    public async Task<GmailConnectionDto> FinishAuthorizationAsync(string state,string code,CancellationToken token)
    {
        ...
        using var form=new FormUrlEncodedContent(new Dictionary<string,string>{{"client_id",configured.ClientId},{"client_secret",configured.ClientSecret},{"code",code},{"code_verifier",pending.Verifier},{"grant_type","authorization_code"},{"redirect_uri",pending.RedirectUri}});
        using var response=await clients.CreateClient("external").PostAsync("https://oauth2.googleapis.com/token",form,token);var tokenBody=await response.Content.ReadFromJsonAsync<JsonElement>(cancellationToken:token);if(!response.IsSuccessStatusCode)throw new InvalidOperationException("Google rejected the authorization code");
        var accessToken=tokenBody.GetProperty("access_token").GetString()??throw new InvalidOperationException("Google did not return an access token");var refreshToken=tokenBody.TryGetProperty("refresh_token",out var refresh)?refresh.GetString():null;if(string.IsNullOrWhiteSpace(refreshToken))throw new InvalidOperationException("Google did not return a refresh token; remove the previous app grant and connect again");var expires=tokenBody.TryGetProperty("expires_in",out var seconds)?seconds.GetInt32():3600;var granted=tokenBody.TryGetProperty("scope",out var scope)?scope.GetString()??"":"";
        ...
        const string sql="""
          INSERT INTO gmail_connections(id,email,archive_id,folder_id,query,ocr_enabled,refresh_token,access_token,access_token_expires_at,status,can_send,can_modify_mailbox,can_manage_calendar,created_at,updated_at)
          VALUES($1,$2,$3,$4,$5,$6,$7,$8,$9,'connected',$10,$11,$12,$13,$13)
    ...
    public async Task DeleteAsync(string id,string owner,CancellationToken token){var record=await GetAsync(id,owner,token)??throw new KeyNotFoundException("Gmail connection not found");if(_runs.TryGetValue(id,out var cancellation))cancellation.Cancel();await using var command=database.CreateCommand("DELETE FROM gmail_connections g USING archives a WHERE g.id=$1 AND a.id=g.archive_id AND a.owner_user_id=$2");command.Parameters.AddWithValue(id);command.Parameters.AddWithValue(owner);await command.ExecuteNonQueryAsync(token);try{using var form=new FormUrlEncodedContent(new Dictionary<string,string>{{"token",record.RefreshToken}});await clients.CreateClient("external").PostAsync("https://oauth2.googleapis.com/revoke",form,token);}catch(HttpRequestException){}}
    ...
    private async Task<string> AccessTokenAsync(GmailConnectionRecord record,CancellationToken token){if(record.AccessToken is not null&&DateTimeOffset.TryParse(record.AccessTokenExpiresAt,out var expires)&&expires>DateTimeOffset.UtcNow.AddMinutes(1))return record.AccessToken;var configured=settings.Current().GmailValue;if(string.IsNullOrWhiteSpace(configured.ClientId)||string.IsNullOrWhiteSpace(configured.ClientSecret))throw new InvalidOperationException("Gmail OAuth credentials are missing; restore them in Admin settings and reauthorize the account");using var form=new FormUrlEncodedContent(new Dictionary<string,string>{{"client_id",configured.ClientId},{"client_secret",configured.ClientSecret},{"refresh_token",record.RefreshToken},{"grant_type","refresh_token"}});using var response=await clients.CreateClient("external").PostAsync("https://oauth2.googleapis.com/token",form,token);var body=await response.Content.ReadFromJsonAsync<JsonElement>(cancellationToken:token);if(!response.IsSuccessStatusCode){var reason=Text(body,"error_description")??Text(body,"error")??$"HTTP {(int)response.StatusCode}";throw new InvalidOperationException($"Google access token refresh failed: {reason}");}var access=body.GetProperty("access_token").GetString()!;var expiresAt=DateTimeOffset.UtcNow.AddSeconds(body.TryGetProperty("expires_in",out var seconds)?seconds.GetInt32():3600).ToString("O");await using var command=database.CreateCommand("UPDATE gmail_connections SET access_token=$2,access_token_expires_at=$3,updated_at=$4 WHERE id=$1");command.Parameters.AddWithValue(record.Public.Id);command.Parameters.AddWithValue(access);command.Parameters.AddWithValue(expiresAt);command.Parameters.AddWithValue(DateTimeOffset.UtcNow.ToString("O"));await command.ExecuteNonQueryAsync(token);return access;}
    private async Task<JsonElement> GoogleJsonAsync(string url,string accessToken,HttpMethod method,JsonElement? body,CancellationToken token){using var request=new HttpRequestMessage(method,url);request.Headers.Authorization=new AuthenticationHeaderValue("Bearer",accessToken);if(body is not null)request.Content=JsonContent.Create(body.Value);using var response=await clients.CreateClient("external").SendAsync(request,token);var text=await response.Content.ReadAsStringAsync(token);if(!response.IsSuccessStatusCode)throw new InvalidOperationException($"Google API returned {(int)response.StatusCode}: {text[..Math.Min(300,text.Length)]}");return string.IsNullOrWhiteSpace(text)?JsonSerializer.SerializeToElement(new{}):JsonDocument.Parse(text).RootElement.Clone();}
